 ...
 char* extract_filename_without_extension(const char* full_filename) 
{
    ...
    if (last_dot != NULL) {
        ...
        filename_without_ext = (char*)malloc((last_dot - full_filename + 1) * sizeof(char));

        // Copy characters before the last dot
        strncpy(filename_without_ext, full_filename, last_dot - full_filename);
        filename_without_ext[last_dot - full_filename] = '\0'; // Null-terminate the string
    } else {
        // If no dot found, just duplicate the full filename
        filename_without_ext = strdup(full_filename);
    }
  ...
  char* result1 = strstr(Lock_status_char, temperatureChar);
   char* result2 = strstr(unLock_status_char, temperatureChar);
    ...
    switch(myInt)
    {
      ...
      case 20: 
             memcpy(&sddata.data20, filename_without_ext, sizeof(filename_without_ext)+1);
...
void Running_Scrn()
{
  ...
  u8g2.drawStr(80,37,"|-");	// write something to the internal memory
 ...
 //const char* char_ptr = " "; // Create a string with single_char and null terminator
  ...
  u8g2.drawStr(5,60,"Ph:7337006557");	// write something to the internal memory
  u8g2.sendBuffer();					// transfer internal memory to the display
  }
...
void Connection_issue_Scrn()
{
  ...
  u8g2.drawStr(10,35,"Check CPU Power");	// write something to the internal memory
  u8g2.drawStr(35,45," And ");	// write something to the internal memory
