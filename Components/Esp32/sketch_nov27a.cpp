...
// Function to print the last pixel of each row
void printLastPixelOfEachRow(File &bodyfile, uint32_t bodywidth, uint32_t totalHeight) {
    // Loop through each row
    for (uint32_t row = 0; row < totalHeight; row++) {
        // Calculate the index of the last pixel in the row
        uint32_t lastPixelIndex = row * bodywidth + (bodywidth - 1);  // Last pixel is bodywidth - 1 in each row
        
        // Seek to the last pixel position
        uint8_t lastPixelValue = readPixel(bodyfile, lastPixelIndex);

        // Print the row number and the last pixel value
        Serial.print("Row ");
        Serial.print(row + 1);  // 1-based row number
        Serial.print(" - Last Pixel: ");
        Serial.println(lastPixelValue);
    }
}
...
void printLastPixelOfAllRows(File &file, uint32_t width, uint32_t height) {
    ...
    for (uint32_t row = 0; row < height; row++) {
        ...
        // Debugging: Print the RGB values for the last pixel of each row
        Serial.print("Row ");
        Serial.print(row + 1);  // Row number (1-based index)
        Serial.print(" RGB: ");
        Serial.print(red);      // Red channel
        Serial.print(", ");
        Serial.print(green);    // Green channel
        Serial.print(", ");
        Serial.println(blue);   // Blue channel

        // Calculate the grayscale value (using red channel here, can also average RGB)
        uint8_t grayscaleValue = red;  // Using the red channel for simplicity
        
        // If the pixel is bright (grayscale > 127), treat it as white (1), otherwise black (0)
        uint8_t grayscaleValueb = (grayscaleValue > 127) ? 0 : 1;

        // Print the result
        Serial.print("Row ");
        Serial.print(row + 1);  // Row number (1-based index)
        Serial.print(" Last Pixel: ");
        Serial.println(grayscaleValueb);  // Grayscale value (1 or 0)
    }

}

uint8_t* cardVars[] = { &e1.myVariable1, &e2.myVariable1, &e3.myVariable1, &e4.myVariable1, &e5.myVariable1, &e6.myVariable1, &e7.myVariable1, &e8.myVariable1, &e9.myVariable1, &e10.myVariable1, &e11.myVariable1, &e12.myVariable1, &e13.myVariable1, &e14.myVariable1, &e15.myVariable1, &e16.myVariable1 ,
...
if (bodyfile && borderfile) {
    ...
    Serial.println(bodyControlPixel);
     ...
     printLastPixelOfEachRow(bodyfile, bodywidth, bodyheight);
...
void separateString(String input)
 {
  ...
  max_connector=strtol(separatedStrings[2].c_str(), &endptr, 10);
      ...
      EEPROM.write(11, max_cardcount); // Write the data to EEPROM
      // EEPROM.commit(); // Commit the data to EEPROM
      // delay(2);
      EEPROM.write(13, max_connector); // Write the data to EEPROM
      EEPROM.commit(); // Commit the data to EEPROM
      delay(2);
  }
...
void separateString3(String input)
 {
  ...
  if (tempString != "") {
    separatedStrings3[index] = tempString;
        ...
        filename_without_ext_fun1 = String(separatedStrings3[3]);
...
Serial.println(result1);
  ...
  Serial.println(result2);
  Serial.println(result1);
  max_cardcount= EEPROM.read(11); // Read the data from EEPROM
  Serial.print("Data read from EEPROM1  max_cardcount: ");
  Serial.println(max_cardcount);
  max_connector= EEPROM.read(13); // Read the data from EEPROM
...
//  char file3[6]="F3";
    ...
    if (deviceConnected) 
    {
       ...
       if(temp_count==0)
       {
        ...
        Serial.print("Selected Files for Design 1513: ");
        ...
        if (pCharacteristic3) {
          ...
          Serial.print("body width and height File for Design 3980: ");
