...
uint16_t read16(File &file) {
  uint16_t result;
  file.read((uint8_t *)&result, sizeof(result));  // Read 2 bytes from the provided file
  return result;
}
...
uint32_t read32() {
  ...
  return result;
   ...
   // Function to read 16-bit value from the BMP header (bit depth)
    uint16_t readBitDepth(File &file) {
    file.seek(28);  // The bit depth is located at byte offset 28 in the BMP header
    return read16(file);  // Use the read16() function to read the bit depth
}
// Function to read a 32-bit value (little-endian format)
// uint32_t read32(File &file) {
//   uint32_t value = 0;
//   for (int i = 0; i < 4; i++) {
//     value |= file.read() << (i * 8);  // Read each byte and shift it into place
//   }
//   return value;
// }
...
void getBMPDimensions(File &file, int &width, int &height) {
  ...
  file.seek(18);
  width = read32();  // Read width (4 bytes)
  
  // Move to the 22nd byte (height starts at byte 22)
  file.seek(22);
  height = read32();  // Read height (4 bytes)
...
// Function to read a pixel from the BMP file (body or border)
uint8_t readPixel(File &file, uint32_t pixelIndex) {
  // Assuming pixels are stored sequentially and starting from offset 54 (header size)
  file.seek(54 + pixelIndex);
  return file.read();  // Read the pixel (assuming 1 byte per pixel)
}
// Array of pointers to the myVariable1 field of each structure
uint8_t* cardVars[] = { &e1.myVariable1, &e2.myVariable1, &e3.myVariable1, &e4.myVariable1, 
                        &e5.myVariable1, &e6.myVariable1, &e7.myVariable1, &e8.myVariable1, 
                        &e9.myVariable1, &e10.myVariable1, &e11.myVariable1, &e12.myVariable1, 
                        &e13.myVariable1, &e14.myVariable1, &e15.myVariable1, &e16.myVariable1 };
...
// Convert body pixel to grayscale value (0 or 1 based on threshold)
    uint8_t grayscaleValue = (bodyPixel == 1) ? 1 : 0;
    Serial.print("grayscalevalue bodypixel: ");
    Serial.println(grayscaleValue);
    byte data = grayscaleValue;
   // Only update if grayscaleValue is 1 (i.e., white pixel)
    if (data == 1) {
      // Ensure card_count is within bounds (0-15)
      if (card_count >= 0 && card_count < 16) {
        // Set the bit for the current card based on the body pixel value
    ...
    // Move to next card
    card_count = (card_count + 1) % 16;  // Reset card_count after 16 cards
    if (card_count == 0) {
  ...
  if (borderPixel == 0 || borderPixel == 1) {
    ...
    // Convert border pixel to grayscale value (0 or 1 based on threshold)
    uint8_t grayscaleValue = (borderPixel == 1) ? 1 : 0;
Serial.print("grayscalevalue border pixel: ");
    Serial.println(grayscaleValue);
    byte data = grayscaleValue;
    // Only update if grayscaleValue is 1 (i.e., white pixel)
    if (data == 1) {
      // Ensure card_count is within bounds (0-15)
      if (card_count >= 0 && card_count < 16) {
        // Set the bit for the current card based on the border pixel value
        *cardVars[card_count] |= (1 << bitToSet); 
        Serial.println(bitToSet); 
  ...
  connector_count = (connector_count + 1) % 16;  // Reset connector_count after 16 connectors
}
void readBMPDimensions2(const char* bodyFilename, const char* borderFilename)
{
  ...
  if (bodyfile && borderfile) {  // Ensure both files are successfully opened
    // Seek to the position where width and height are stored
    bodyfile.seek(18);  
    borderfile.seek(18);
    // Read width and height of the body image
    getBMPDimensions(bodyfile, bodywidth, bodyheight);
    // Read width and height of the border image (optional)
    getBMPDimensions(borderfile, borderwidth, borderheight);

    // Print out the dimensions
    Serial.print("Body Width: ");
    Serial.println(bodywidth);
    Serial.print("Body Height: ");
    Serial.println(bodyheight);
    Serial.print("Border Width: ");
    Serial.println(borderwidth);
    Serial.print("Border Height: ");
    Serial.println(borderheight);
  } else {
...
if (!bodyfile && !borderfile) {
    Serial.println("Failed to open BMP files");
    return;
  }

  // Read and validate BMP header
  if (read16(bodyfile) != 0x4D42) {
    Serial.println("Not a valid BMP file");
    bodyfile.close();
    return;
  }
   // Read and validate BMP header
  if (read16(borderfile) != 0x4D42) {
    Serial.println("Not a valid BMP file");
    borderfile.close();
    return;
  } 
  if (bodyfile && borderfile) {
    ...
    uint16_t bodyBitDepth = readBitDepth(bodyfile);
    uint16_t borderBitDepth = readBitDepth(borderfile);
    if (bodyBitDepth != 1 || borderBitDepth != 1) {
    Serial.println("Only 1-bit BMP files are supported");
    bodyfile.close();
    borderfile.close();
    return;
  }
    bodyfile.seek(54 + bodywidth);  // Skip the header (54 bytes) and go to pixel 1537 (index 1536)

    // Read the 1537th pixel (control pixel)
    uint8_t bodyControlPixel = readPixel(bodyfile, bodywidth);  // 1537th pixel
    ...
    for (int y = 0; y < 1; y++) {  // Loop through rows (1 row in this example)
       // Initialize connector count, card count, and bit position
        ...
        Serial.print("prev=");
        Serial.println(prev);
      for (int x = 0; x < bodywidth; x++) {  // Process the first 1536 pixels
        ...
        present = prev + 3;
        bodyfile.seek(present); // Move to the start of pixel data
        prev = present;
        // Read the body pixel
        int bodyPixel = (readPixel(bodyfile, x) > 127) ? 1 : 0;

        // Read the border pixel, but only if the border design is enabled (runBorderDesign)
        uint8_t borderPixel = (runBorderDesign) ? (readPixel(borderfile, x) > 127 ? 1 : 0) : 0;

        // Process the connector for both body and border pixels
        processConnector(bodyPixel, borderPixel, card_count, connector_count, bitToSet);
      }
    }
        Serial.println();
...
void separateString(String input)
 {
  ...
  max_cardcount=strtol(resultseparatedStrings[1].c_str(), &endptr, 10);
  max_connector=strtol(resultseparatedStrings[2].c_str(), &endptr, 10);
...
void separateString2(String input)
 {
  ...
  if( height_var!=strtol(resultseparatedStrings[1].c_str(), &endptr, 10) && strtol(resultseparatedStrings[1].c_str(), &endptr, 10)!=0)
  {
      ...
      EEPROM.write(21, height_var); // Write the data to EEPROM
      EEPROM.commit(); // Commit the data to EEPROM
...
void separateString3(String input)
 ...
 filename_without_ext_fun1 = String(resultseparatedStrings[3]);  
...
void setup() {
  ...
  pAdvertising->setMinPreferred(0x06);  // Set min advertising interval
  pAdvertising->setMaxPreferred(0x12);  // Set max advertising interval
  ...
  char ch;
  int index = 0;
  while ((ch = EEPROM.read(address)) != '\0' && index < 100) { // Read until null character or EEPROM size limit
    readData[index] = ch;
    Serial.println("eeprom read value22 result2");
    address++;
    index++;
  }
  ...
  while ((ch = EEPROM.read(address1)) != '\0' && index1 < 100) { // Read until null character or EEPROM size limit
    readData1[index1] = ch;
    Serial.println("eeprom read value22 result1");
    address1++;
    index1++;
  }
  ...
  Serial.println(sizeof(readData1));
  result1=string1 + readData1 + string3;  // this is for mobile app
  Serial.println(result1);
  Serial.print("eeprom read value=");
  Serial.println(result2);
  Serial.println(result1);
  max_cardcount = EEPROM.read(12); // Read the data from EEPROM
  Serial.print("Data read from EEPROM1  max_cardcount: ");
  Serial.println(max_cardcount);
  max_connector = EEPROM.read(14); // Read the data from EEPROM
...
void loop() {
  ...
  if (deviceConnected) {
    ...
    if (rqsNotify) {
      ...
      if (temp_count == 0) {
        ...
        Serial.print("Selected File for Design 1513: ");
        ...
        if (pCharacteristic22) {
          ...
          Serial.print("card and connector count for Design 1535: ");
   ...
   Serial.println("Received Data is:");
      ...
      } else if (result22 != nullptr) // delete file option 
      {
        ...
        if (result11 != nullptr) //
        {
          ...
          string2 = filename_without_ext_fun;
          string22 = filename_without_ext_fun1;
          String newValue2 = String(string2.c_str());
          String newValue1 = String(string22.c_str());
        ...
        if ((newValue != previousValue || temp_count2 == 0) && (newValue1 != previousValue1 || temp_count2 == 0) && (file_check == 1)) {
          ...
          previousValue1 = string22;
          Serial.println("String22 File:");
          Serial.print(string22);
          ...
          int length1 = strlen(stringValue1);
           ...
           for (int i = 0; i < length1; i++) { 
            EEPROM.write(60 + i, previousValue1[i]);
            Serial.print(previousValue1);
            }
            EEPROM.write(60 + length1, '\0'); // Correct null-termination for the second string
            EEPROM.commit();
            Serial.println("result1");
            ...
            result1 = string1 + previousValue1 + string3;
            Serial.println("result1");
            Serial.print(result1);
            Serial.println("result2");
            Serial.print(result2);
  ...
  if (mem_cpy_set == 1 && mode_set == 0) {
    ...
    readBMPDimensions(result1.c_str(), result2.c_str());
    mem_cpy_set = 0;
  }
  data_process();
  if (sensor1NotDetected == true || sensor2NotDetected == true) {
